...
callableInput(Ctx,[S|Tring],Input,Output):- maplist(atomic,[S|Tring]),joinAtoms([S|Tring],' ',String),!,callableInput0(Ctx,String,Input,Output).
...
callableInput0(Ctx,Atom,_Input,VotesO-Output):-atom_prefix(Atom,'@'),
  % re-direct to input
  locally(set_prolog_flag(debug,true),
...
subclassMakeUserDict(Ctx,UserDict,SYM):-debugFmt(x(subclassMakeUserDict(Ctx,UserDict,SYM))),addInherit(UserDict,SYM).
...
toTAtom(Text,Atom):-trace,atom_number(Atom,Text).
